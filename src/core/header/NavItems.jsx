...
const NavItems = ({ ToggleMenu, MenuOpen, isActive, }) => {
  ...
  return (
    <>
      <nav className={`${MenuOpen ? 'translate-x-0' : '-translate-y-full'} duration-300 mx-auto text-[#40C3CD]  poppins-medium bg-white  min-h-65 max-h-screen max-w-full w-full top-0 right-0 absolute z-50`}>

        <div className="p-5">

          <div className="w-44  p-2"></div>
          <div onClick={ToggleMenu} className="absolute right-5 top-5 cursor-pointer"><RxCross2  className="text-2xl" /></div>

          <ul className="flex flex-col items-start mt-5 *:my-2 *:px-2  overflow-y-auto max-h-[calc(100vh-150px)] ">
            <Link to="/" className={` ${isActive == '/' ? 'text-white bg-[#40C3CD]  rounded-lg min-w-[75px] py-1  px-2' : ' '}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
                }}
            ...
            <li ref={dropdown1Ref} >
              <Link onClick={() => setDropdown1(!dropdown1)} className={`flex items-center gap-1  ${about.some(link => location.pathname === link.path)
                ? 'text-white bg-[#40C3CD]   py-1 px-1 rounded-lg'
                : ''
                }`}>
                About Us
                <FaChevronDown
            ...
            <Link to="/contact-us" className={` ${isActive == '/contact-us' ? 'text-white bg-[#40C3CD]  rounded-lg min-w-[75px] py-1  px-2' : ''}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
